...
However, prior work often evaluates efficiency techniques in isolation, making it difficult to systematically evaluate architectural compression methods and inference-time optimizations under consistent experimental conditions.

In this project, we investigate energy–accuracy trade-offs in transformer-based NLP models using a controlled benchmarking framework that measures GPU power consumption during both training and inference. Using the [SQuAD v2](https://huggingface.co/datasets/rajpurkar/squad_v2) question answering benchmark, we evaluate several BERT-derived architectural variants alongside inference-time optimizations within a unified experimental setting.
...
Finally, the most energy-efficient strategy depends on the expected workload: architectures with lower training energy are preferable when inference volume is small, whereas configurations with lower per-example inference energy dominate at large deployment scales.
