...
def extract_model_point_from_summary(summary_json_path: Path, label: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "training_energy_kj": float(
            payload["training_energy"]["energy_joules"]
        ) / 1000.0,
    }
...
# Plot one point per model for the architecture-level training-energy vs. accuracy chart
def plot_model_architecture_training_energy(
    model_points: List[Dict[str, Any]],
    output_path: Path,
    plot_title: str,
    color_map_name: str,
    ...
    x_values = [float(model_point["training_energy_kj"]) for model_point in model_points]
    x_min = min(x_values)
    x_max = max(x_values)
    x_margin = 0.05 * (x_max - x_min) if x_max > x_min else 1000.0

    figure = plt.figure(figsize=(8, 6))

    for model_point in model_points:
        label = str(model_point["label"])
        x_value = float(model_point["training_energy_kj"])
    ...
    plt.xlabel("Total Training Energy (kJ)")
    plt.ylabel("F1")
...
def build_argument_parser() -> argparse.ArgumentParser:
    argument_parser = argparse.ArgumentParser(
        description="Plot the architecture-level training-energy vs. accuracy chart."
    )
    ...
    argument_parser.add_argument(
        "--plot-title",
        default="Training Energy–Accuracy Trade-offs Across Model Architectures",
...
def main() -> None:
    ...
    plot_model_architecture_training_energy(
        model_points=model_points,
