...
from src.evaluation.pareto_plots import (
    ...
    plot_precision_sweep_comparison,
    plot_sequence_length_sweep_comparison,
    plot_single_precision_sweep,
    plot_single_sequence_length_sweep,
    plot_single_token_pruning_sweep,
    plot_token_pruning_sweep_comparison,
...
# Run inference precision sweep for one checkpoint
def run_squad_v2_precision_sweep(arguments: argparse.Namespace) -> None:
    dataset_config = read_yaml_file(arguments.dataset_config_path)
    model_config = read_yaml_file(arguments.model_config_path)
    inference_config = read_yaml_file(arguments.inference_config_path)
    ...
    run_identifier = arguments.run_identifier or create_timestamped_run_identifier(
        f"precision_sweep_{sanitized_model_label}"
    )
    run_directory = ensure_directory_exists(Path("runs") / run_identifier)
    log_file_path = run_directory / "logs.txt"
    ...
    for precision_mode in inference_config["precision_modes"]:
        normalized_precision_mode = str(precision_mode).strip().lower()

        sub_run_directory = ensure_directory_exists(
            run_directory / f"precision_{normalized_precision_mode}"
        )

        append_line_to_text_file(
            log_file_path,
            f"[precision={normalized_precision_mode}] loading model",
        )

        # Reload model fresh for each precision mode
        qa_artifacts = load_question_answering_model_and_tokenizer(
            model_name_or_path=arguments.checkpoint_path,
        ...
        append_line_to_text_file(
            log_file_path,
            f"[precision={normalized_precision_mode}] starting evaluation",
        )

        evaluation_result = evaluate_checkpoint_on_squad_v2_at_precision(
            run_directory=sub_run_directory,
            log_file_path=log_file_path,
            model=model,
            tokenizer=tokenizer,
        ...
        sweep_rows.append(
            {
                "model_label": model_label,
                "checkpoint_path": arguments.checkpoint_path,
                "precision_mode": evaluation_result["precision_mode"],
                "precision_supported": evaluation_result["precision_supported"],
                "precision_skip_reason": evaluation_result["precision_skip_reason"],
                "model_parameter_dtype": evaluation_result["model_parameter_dtype"],
                "maximum_sequence_length": int(evaluation_result["maximum_sequence_length"]),
                "effective_document_stride": int(evaluation_result["effective_document_stride"]),
                "exact": (
                    float(metrics_thresholded["exact"])
                    if metrics_thresholded is not None
                    else None
                ),
                "f1": (
                    float(metrics_thresholded["f1"])
                    if metrics_thresholded is not None
                    else None
                ),
                "inference_energy_joules": evaluation_result["inference_energy_joules"],
                "joules_per_inference_example": evaluation_result["joules_per_inference_example"],
                "joules_per_feature_window": evaluation_result["joules_per_feature_window"],
                "joules_per_inference_token": evaluation_result["joules_per_inference_token"],
                "joules_per_exact_match_correct_example": evaluation_result["joules_per_exact_match_correct_example"],
                "average_latency_per_raw_example_milliseconds": evaluation_result["average_latency_per_raw_example_milliseconds"],
                "average_latency_per_feature_window_milliseconds": evaluation_result["average_latency_per_feature_window_milliseconds"],
                "number_of_raw_evaluation_examples": evaluation_result["number_of_raw_evaluation_examples"],
                "number_of_feature_windows": evaluation_result["number_of_feature_windows"],
                "number_of_inference_tokens": evaluation_result["number_of_inference_tokens"],
            }
        )

        if metrics_thresholded is None:
            append_line_to_text_file(
                log_file_path,
                f"[precision={normalized_precision_mode}] skipped "
                f"reason={evaluation_result['precision_skip_reason']}",
            )
        else:
            append_line_to_text_file(
                log_file_path,
                f"[precision={normalized_precision_mode}] done "
                f"f1={metrics_thresholded['f1']:.4f} "
                f"exact={metrics_thresholded['exact']:.4f} "
                f"joules_per_inference_token={evaluation_result['joules_per_inference_token']:.8f}",
            )
    ...
    plot_single_precision_sweep(
        ...
        x_axis_label="Energy (J / Token)",
        y_axis_label="F1",
        plot_title=f"Inference Precision Sweep - {model_label}",
    )
...
# Run dynamic token-pruning sweep for one checkpoint
def run_squad_v2_token_pruning_sweep(arguments: argparse.Namespace) -> None:
    dataset_config = read_yaml_file(arguments.dataset_config_path)
    model_config = read_yaml_file(arguments.model_config_path)
    inference_config = read_yaml_file(arguments.inference_config_path)
    ...
    run_identifier = arguments.run_identifier or create_timestamped_run_identifier(
        f"token_pruning_sweep_{sanitized_model_label}"
    )
    run_directory = ensure_directory_exists(Path("runs") / run_identifier)
    log_file_path = run_directory / "logs.txt"
    ...
    # Load raw evaluation split once and reuse it across all keep-ratio values
    raw_evaluation_split = load_raw_squad_v2_evaluation_split(
        huggingface_dataset_id=dataset_config["huggingface_dataset_id"],
        evaluation_split_name=dataset_config["evaluation_split_name"],
        maximum_evaluation_examples=dataset_config.get("maximum_evaluation_examples"),
    ...
    for token_pruning_keep_ratio in inference_config["token_pruning_keep_ratios"]:
        ...
        append_line_to_text_file(
            log_file_path,
            f"[token_pruning_keep_ratio={keep_ratio_label}] done "
            f"f1={metrics_thresholded['f1']:.4f} "
            f"exact={metrics_thresholded['exact']:.4f} "
            f"joules_per_inference_token={evaluation_result['joules_per_inference_token']:.8f}",
        )
    ...
    plot_single_token_pruning_sweep(
        ...
        x_axis_label="Energy (J / Token)",
        y_axis_label="F1",
        plot_title=f"Dynamic Token Pruning Sweep - {model_label}",
    )
...
# Plot an overlay comparison across several already-generated precision sweep CSV files
def plot_squad_v2_precision_sweep_comparison(arguments: argparse.Namespace) -> None:
    output_path = Path(arguments.output_path)
...
# Plot an overlay comparison across several already-generated token-pruning sweep CSV files
def plot_squad_v2_token_pruning_sweep_comparison(arguments: argparse.Namespace) -> None:
    output_path = Path(arguments.output_path)
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    precision_parser = subparsers.add_parser(
        "run_squad_v2_precision_sweep",
        help="Run an inference precision sweep for one checkpoint on SQuAD v2.",
    )
    precision_parser.add_argument("--dataset-config-path", required=True)
    precision_parser.add_argument("--model-config-path", required=True)
    precision_parser.add_argument("--inference-config-path", required=True)
    precision_parser.add_argument("--checkpoint-path", required=True)
    precision_parser.add_argument("--model-label", required=True)
    precision_parser.add_argument("--run-identifier", default=None)
    precision_parser.set_defaults(handler=run_squad_v2_precision_sweep)

    token_pruning_parser = subparsers.add_parser(
        "run_squad_v2_token_pruning_sweep",
        help="Run a dynamic token-pruning inference sweep for one checkpoint on SQuAD v2.",
    )
    token_pruning_parser.add_argument("--dataset-config-path", required=True)
    token_pruning_parser.add_argument("--model-config-path", required=True)
    token_pruning_parser.add_argument("--inference-config-path", required=True)
    token_pruning_parser.add_argument("--checkpoint-path", required=True)
    token_pruning_parser.add_argument("--model-label", required=True)
    token_pruning_parser.add_argument("--run-identifier", default=None)
    token_pruning_parser.set_defaults(handler=run_squad_v2_token_pruning_sweep)
    ...
    precision_comparison_parser = subparsers.add_parser(
        "plot_squad_v2_precision_sweep_comparison",
        help="Overlay several precision sweep CSV files in one comparison figure.",
    )
    precision_comparison_parser.add_argument("--pareto-csv-paths", nargs="+", required=True)
    precision_comparison_parser.add_argument("--model-labels", nargs="+", required=True)
    precision_comparison_parser.add_argument("--output-path", required=True)
    precision_comparison_parser.add_argument("--x-field-name", default="joules_per_inference_token")
    precision_comparison_parser.add_argument("--y-field-name", default="f1")
    precision_comparison_parser.add_argument("--x-axis-label", default="Energy (J / Token)")
    precision_comparison_parser.add_argument("--y-axis-label", default="F1")
    precision_comparison_parser.add_argument(
        "--plot-title",
        default="Inference Precision Sweep Comparison",
    )
    precision_comparison_parser.set_defaults(
        handler=plot_squad_v2_precision_sweep_comparison
    )

    token_pruning_comparison_parser = subparsers.add_parser(
        "plot_squad_v2_token_pruning_sweep_comparison",
        help="Overlay several token-pruning sweep CSV files in one comparison figure.",
    )
    token_pruning_comparison_parser.add_argument("--pareto-csv-paths", nargs="+", required=True)
    token_pruning_comparison_parser.add_argument("--model-labels", nargs="+", required=True)
    token_pruning_comparison_parser.add_argument("--output-path", required=True)
    token_pruning_comparison_parser.add_argument("--x-field-name", default="joules_per_inference_token")
    token_pruning_comparison_parser.add_argument("--y-field-name", default="f1")
    token_pruning_comparison_parser.add_argument("--x-axis-label", default="Energy (J / Token)")
    token_pruning_comparison_parser.add_argument("--y-axis-label", default="F1")
    token_pruning_comparison_parser.add_argument(
        "--plot-title",
        default="Dynamic Token Pruning Sweep Comparison",
    )
    token_pruning_comparison_parser.set_defaults(
        handler=plot_squad_v2_token_pruning_sweep_comparison
    )   
