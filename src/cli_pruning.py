...
def run_prune_and_refinetune_squad_v2(arguments: argparse.Namespace) -> None:
    ...
    pruning_config = read_yaml_file(arguments.pruning_config_path)

    # Create run directory and write resolved configuration for reproducibility
    run_identifier = arguments.run_identifier or create_timestamped_run_identifier("prune_refinetune_squad_v2")
    ...
    append_line_to_text_file(
        log_file_path,
        f"[configs] dataset={arguments.dataset_config_path} model={arguments.model_config_path} "
        f"training={arguments.training_config_path} pruning={arguments.pruning_config_path}",
    )

    # Set random seed for reproducibility
    set_seed(int(training_config["random_seed"]))

    # Load tokenizer and model weights from the checkpoint path
    ...
    # Apply magnitude pruning before refinetuning
    append_line_to_text_file(log_file_path, "[pruning] applying global magnitude pruning")
    pruning_report = apply_global_magnitude_pruning(
        model=model,
        target_sparsity=float(pruning_config["target_sparsity"]),
        prune_linear_layers=bool(pruning_config.get("prune_linear_layers", True)),
        prune_question_answering_head=bool(pruning_config.get("prune_question_answering_head", True)),
        remove_pruning_reparameterization=bool(pruning_config.get("remove_pruning_reparameterization", True)),
    )
    write_json_file(pruning_report, run_directory / "pruning_report.json")
    append_line_to_text_file(log_file_path, f"[pruning] report: {pruning_report}")

    # Load raw SQuAD v2 train/evaluation splits (optionally truncated)
    ...
    append_line_to_text_file(log_file_path, "[energy][train] starting energy meter (refinetune)")
    ...
    # Measure inference energy for evaluation predict
    ...
    # Extract raw predictions from Trainer output
    ...
    # Postprocess to get span predictions and no-answer probabilities
    ...
    append_line_to_text_file(log_file_path, "[done] prune + refinetune run complete")
    print(f"Run complete: {run_directory}")
    print(f"Summary written to: {run_directory / 'summary.json'}")

# Build CLI argument parser for pruning runs
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="NLP Frontiers - Pruning CLI (separate from main harness)")
    subparsers = parser.add_subparsers(dest="command", required=True)

    # Create command for pruning + refinetune baseline on SQuAD v2
    pruning_parser = subparsers.add_parser(
        "prune_and_refinetune_squad_v2",
        help="Load a checkpoint, apply magnitude pruning, briefly refinetune, and evaluate on SQuAD v2.",
    )
    pruning_parser.add_argument("--dataset-config-path", required=True)
    pruning_parser.add_argument("--model-config-path", required=True)
    pruning_parser.add_argument("--training-config-path", required=True)
    pruning_parser.add_argument("--pruning-config-path", required=True)
    pruning_parser.add_argument("--checkpoint-path", required=True)
    pruning_parser.add_argument("--run-identifier", default=None)
    pruning_parser.set_defaults(handler=run_prune_and_refinetune_squad_v2)

    return parser

# Entrypoint for running the pruning CLI
def main() -> None:
