...
from src.models.modifiers import apply_bert_freezing_strategy_from_config, summarize_trainable_parameters
...
def run_squad_v2_baseline_training_and_evaluation(arguments: argparse.Namespace) -> None:
    ...
    # Apply an optional freezing strategy
    try:
        freezing_summary = apply_bert_freezing_strategy_from_config(model=model, training_config=training_config)
        append_line_to_text_file(log_file_path, f"[model] {freezing_summary}")
    except ValueError as error:
        # If the model is not BERT-style, or config is invalid, fail fast with a clear message.
        raise

    # Log trainable parameter counts for sanity-checking
    trainable_summary = summarize_trainable_parameters(model)
    append_line_to_text_file(
        log_file_path,
        f"[model] trainable_parameters={trainable_summary.trainable_parameters} "
        f"total_parameters={trainable_summary.total_parameters} "
        f"trainable_fraction={trainable_summary.trainable_fraction:.6f}"
    )
